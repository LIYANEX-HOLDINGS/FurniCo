...
function ShopContent() {
  ...
  if (!isClient) return <div className="min-h-screen flex items-center justify-center font-albert-sans text-xl animate-pulse">{t('loading_store')}</div>;

  return (
    <div className="py-12 md:py-20 bg-background min-h-screen">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="mb-10 text-center">
          <h1 className="text-3xl md:text-5xl font-bold font-albert-sans mb-4">{t('shop')}</h1>
          <p className="text-text-muted font-roboto">{t('latest_description')}</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-1/4 space-y-12">
            <div>
              <h3 className="text-xl font-bold font-albert-sans mb-6 pb-2 border-b border-gray-100">{t('useful_links')}</h3>
              <div className="flex flex-col gap-4">
                {categories.map((cat) => (
                  ...
                  >
                    <span>{cat === "all" ? t('view_all') : t(cat.toLowerCase())}</span>
            ...
            <div className="hidden lg:block">
              <h3 className="text-xl font-bold font-albert-sans mb-6 pb-2 border-b border-gray-100">{t('filter_price')}</h3>
          ...
          <div className="flex-1 space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 py-4 border-b border-gray-100">
              <span className="text-sm font-bold font-albert-sans text-gray-400 uppercase tracking-widest">
                {t('showing_results').replace('%d', filteredProducts.length.toString())}
              </span>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2 cursor-pointer group hover:text-primary transition-colors">
                  <span className="text-sm font-bold font-albert-sans uppercase">{t('sort_by')}: {sortBy === 'default' ? t('default_sort') : sortBy}</span>
            ...
            {filteredProducts.length === 0 && (
              <div className="py-20 text-center">
                ...
                <h3 className="text-2xl font-bold font-albert-sans text-gray-300">{t('no_products')}</h3>
                <p className="text-gray-400 font-roboto">{t('try_another')}</p>
              </div>
            )}
          </div>
        </div>
...
export default function ShopPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-albert-sans text-xl animate-pulse">Loading...</div>}>
      <ShopContent />
    </Suspense>
  );
}
