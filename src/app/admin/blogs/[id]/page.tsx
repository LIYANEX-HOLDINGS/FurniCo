...
import { ArrowLeft, Save, Loader2, Link as LinkIcon, Check, Trash2 } from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 font-albert-sans">
      <div className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Log (Post)</h1>
          <p className="text-sm text-gray-500">Modify your existing blog post.</p>
        </div>
      </div>
          ...
          <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-gray-900 font-bold"
              />
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full bg-primary text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-primary/30 disabled:opacity-50"
            >
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
              {isLoading ? "Saving..." : "Update Post"}
            </button>
            {error && <p className="text-xs text-red-500 text-center font-medium bg-red-50 p-3 rounded-lg border border-red-100">{error}</p>}
          </div>
        </div>
      </form>
    </div>
